...
func main() {
	...
	srv1 := &http.Server{Addr: ":50005", Handler: app}
	go srv1.ListenAndServe()
	println("Start a server listening on http://localhost:50005")

	// start a "second-secondary" server listening on localhost:5050.
	//
	// http://localhost:5050/
	// http://localhost:5050/mypath
	srv2 := &http.Server{Addr: ":50006", Handler: app}
	go srv2.ListenAndServe()
	println("Start a server listening on http://localhost:50006")
	...
	app.Run(iris.Addr(":50002")) // Block here.
}
